...
日志同时输出到 stderr 和日志文件（10 MB 自动轮转，保留 5 个备份）。设置为 `"debug"` 级别可查看每次 API 请求详情。
...
每个插件通过元数据声明自己的类型。目前我们提供了 Rust SDK（`plugin/sdk/rust/`）封装了 FFI 契约，但 ABI 足够简单，任何语言都能直接实现。
...
完整示例见 `examples/plugin/`。Rust SDK：`plugin/sdk/rust/`。
