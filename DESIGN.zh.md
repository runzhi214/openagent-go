...
nil = 全部放行。
	...
	- `examples/webui/main.go` — Web UI，SSE + 浏览器弹窗确认
...
│   └── plan.go               Plan 类型 + Run/RunStream + PlanOption
│
...
│   ├── plan_handler.go       PlanHandler：Plan 会话 CRUD + 生成/编辑/执行 + SSE
│   ├── types.go              SessionInfo + 请求/响应类型 + SSEEvent
│   └── sse.go                SSE 工具（writeSSE, setSSEHeaders）
│
├── runner/
│   └── acp/
│       └── runner.go         ACP Runner：external agent 作为 Team 成员
...
└── examples/
    ...
    └── webui/                Web UI 示例（SSE 流式 + 审批 + team + plan 模式）
```
...
| ⑥ | Approver | ✅ | TUI (bubbletea v2) + WebUI (SSE)，human-in-the-loop |
...
`AgentInfo.Type` 在 Team 构造 `AgentInfo` 时自动填充（`WithTeamAgent` 设 `AgentInternal`，`AddAgent` 根据 runner 类型自动判断）。类型信息已流通到 Router、Team prompt（显示 `[internal]` / `[external]`）、Plan Planner 和 WebUI。
...
✅ 已实现。`plan/` 包提供目标分解 → DAG 依赖图 → 拓扑排序并行执行 → 失败自动 Replan。

Plan 是继 Team 之后的第二种多 Agent 编排模式：

| | Team | Plan |
|------|------|------|
| 决策时机 | 运行时，Agent 自主 Handoff | 执行前，Planner 生成 DAG |
| 并行 | 无（串行 handoff 链） | 拓扑批次自动并行 |
| 失败处理 | Agent 自己兜底 | 局部子树 Replan |
| 确定性 | 低（Agent 可能不 Handoff） | 高（DAG 锁死执行顺序） |
...
planner := plan.NewLLMPlanner(model)
```
...
AutoReplan=false (REST/WebUI 默认):
  Step X 失败
...
**DAG 校验（[plan/validate.go](plan/validate.go)）：**
...
p := plan.NewPlan(
    ...
    plan.WithAutoReplan(false), // 可选：失败时暂停等人工介入
)
...
Single 页面输入 `/plan <goal>` → 立即显示 "Generating plan..." 占位 → Planner 生成 DAG → 替换为 SVG 连线图卡片。

Pre-execution 四个按钮：`[▶ Execute] [✏️ Edit] [🔄 Replan] [✕ Cancel]`

- **Edit**: modal 编辑 Plan JSON → Save → 重新渲染 DAG
- **Replan**: 可选填自然语言 feedback → 模型基于当前 plan + feedback 重新生成
- **Cancel**: 清除 DAG，回到普通聊天

执行期间实时更新卡片状态（pending→running→done/failed），点击卡片弹出 Agent 完整输出（含 tool 调用记录），内容实时更新。

**REST API（[rest/plan_handler.go](rest/plan_handler.go)）：**
...
ph.Register(mux)
// POST   /plan/sessions                         — 创建 session
// POST   /plan/sessions/{id}/generate           — {goal} → PlanDef
// GET    /plan/sessions/{id}/plan               — 获取当前 PlanDef
// PUT    /plan/sessions/{id}/plan               — 用户编辑 PlanDef
// POST   /plan/sessions/{id}/execute            — 触发执行（202）
// GET    /plan/sessions/{id}/events             — SSE 流式进度（pause 期间保持连接）
// POST   /plan/sessions/{id}/cancel             — 取消执行
// POST   /plan/sessions/{id}/approve            — 工具审批
// POST   /plan/sessions/{id}/steps/{stepID}/retry — 手动重试失败步骤
// POST   /plan/sessions/{id}/replan             — {feedback} → 带用户反馈的 Replan
```
...
**WebUI `/goal` 命令：** Chat 页面输入 `/goal <目标>` → 取消正在运行的 plan → 用 Clone + MaxTurns=50 启动 Goal mode → SSE 实时推送进度。
...
**用途：**
- WebUI 多 tab 同步：用户消息、Agent 响应、会话删除、审批状态跨 tab 同步
...
- `examples/webui/` — Go HTTP + 内嵌 HTML/JS，SSE 流式 + 审批弹窗
  - `/` — 单 Agent 对话 + `/plan` 命令（目标 → DAG → 执行）+ `/goal` 命令（自主目标模式）
  - `/team` — 多 Agent Team 编排，handoff 可视化
